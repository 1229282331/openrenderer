...
{
    for(int i=0; i<num_threads; i++)
        m_pipeline[i] = std::make_unique<Pipeline>(primitive, freq, vertexShaderFunc, fragmentShaderFunc, m_framebuffers.get());
}
...
void Render::drawFrame(const Loader& obj_loader)
{
    ...
    {
        int use_threads = std::clamp(int(obj.indices.size())/3, 1, 20);
        for(int i=0; i<num_threads; i++)
        {
